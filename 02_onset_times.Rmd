...
mean_rel_eye_size_ebird <- read.csv ("results/mean_rel_eye_size_ebird.csv") # error: file not opening/wrong path
```
...
summer_rel_onset_times <- summer_rel_onset_times %>%
  left_join (., mean_rel_eye_size_ebird, by = c('scientific_name', 'common_name')) %>%
  dplyr::select (species, median_begin_time, median_relative_onset_time, scientific_name, common_name, Order, eBird_codes, rel_mean_eye_size, q1, q3) #only keeping columns which are required for further analyses

#winter
winter_rel_onset_times <- winter_rel_onset_times %>%
  left_join (., mean_rel_eye_size_ebird, by = c('scientific_name', 'common_name')) %>%
  dplyr::select (species, median_begin_time, median_relative_onset_time, scientific_name, common_name, Order, eBird_codes, rel_mean_eye_size, q1, q3) 
```
...
summer_begin_time <- summer_rel_onset_times %>%
  ...
  mutate (season = 'summer') %>% #so that the dataframes can be combined together without confusion
...
winter_begin_time <- winter_rel_onset_times %>%
  mutate (colour = case_when (median_begin_time < as_hms ('05:42:15') ~ '#FFB000',
         ...
         median_begin_time > as_hms ('07:00:01') & median_begin_time <= as_hms ('08:00:00') ~ '#FE6100',
         median_begin_time > as_hms ('08:00:01') & median_begin_time <= as_hms ('09:00:00') ~ '#785EF0')) %>%
  mutate(label_coloured = paste0("<span style = 'colour: ", colour, ";'>", common_name, "</span>")) %>% 
  arrange (colour) %>% 
...
summer_begin_time$species_index <- as.numeric(fct_reorder(summer_begin_time$label_coloured,                                                      summer_begin_time$median_begin_time))

#summer_begin_time$eye_x <- summer_begin_time$species_index -10


# including changes to the visualization code below:

# visualization
# re-ordering the data and create styled labels

summer_begin_time <- summer_begin_time %>%
  arrange(median_begin_time) %>%
  mutate(
    # creating the HTML-styled labels for coloring
    common_name_styled = paste0("<span style='color:", colour, "'>", common_name, "</span>"),
...
summer_onset_times_plot <- ggplot(summer_begin_time, 
  ...
  # remove faceting (potentially causing issue)
  # facet_grid(common_name ~ median_begin_time, scales = 'free_x') + 
...
ggsave(summer_onset_times_plot,
  filename = "figs/trial.png",  width    = 7,
  height   = 12,
  units    = "in",
  dpi      = 300
)
...
winter_onset_times_plot <- ggplot (winter_begin_time, aes (x = fct_reorder (label_coloured, median_begin_time), y = median_begin_time, color = colour)) +
  geom_point (shape = 20, size = 4) +
  #geom_point (aes(x = rel_mean_eye_size)) +
  geom_hline(yintercept = winter_median_nautical_dawn, size = 0.3) + 
  annotate("text", x = 75, y = as_hms("05:42:15"), label = "Median nautical dawn", 
           vjust = -0.5, hjust = 1.2,
  ...
  breaks = hms::as_hms(c("05:00:00", "06:00:00", "07:00:00", "08:00:00", "08:30:00")))+
  scale_x_discrete (labels = winter_begin_time$common_name) + 
  scale_color_manual (values = c('#000000', '#009E73', '#785EF0','#FE6100'),
                      labels = c('6 am to 7 am', 'nautical dawn to 6 am', '8 am to 9 am', '7am to 8 am')) +
  labs (y = 'Median time of first dawn vocalization', x = 'Species common name') +
  theme(axis.text.y = element_text (color = winter_begin_time$colour), 
