...
mean_rel_eye_size_ebird <- read.csv ("results/mean_rel_eye_size_ebird.csv")
```
...
summer_begin_time$species_index <- as.numeric(fct_reorder(summer_begin_time$label_coloured,
                                                          summer_begin_time$median_begin_time))

summer_begin_time$eye_x <- summer_begin_time$species_index -10

summer_onset_times_plot <- ggplot (summer_begin_time, aes (x = fct_reorder (label_coloured, median_begin_time), y = median_begin_time, color = colour)) +
  geom_point (shape = 20, size = 4) +
  #geom_point (aes(x = rel_mean_eye_size)) +
  geom_hline(yintercept = summer_median_nautical_dawn, size = 0.3) + 
  annotate("text", x = 85, y = as_hms("05:42:26"), label = "Median nautical dawn", 
           vjust = -0.5, hjust = 1.3, 
  ...
  scale_size_continuous(name = "Relative Eye Size") +
