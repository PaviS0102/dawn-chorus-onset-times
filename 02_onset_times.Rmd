...
```{r}
ggplot(summer_begin_time, aes(x = median_relative_onset_time, y = rel_mean_eye_size)) +
  ...
  theme_bw () +
  labs(y = 'Mean eye size relative to body mass', x = 'Median relative onset times') +
  theme(axis.text.y = element_markdown(family = "Century Gothic", size = 10),
    axis.text.x = element_text(family = "Century Gothic", size = 10), 
    text = element_text(family = "Century Gothic"))

x <- ggplot(summer_begin_time, aes(x = 1, y = common_name_ordered)) +
  geom_point(aes(size = mean_eye_size, color = colour)) +
  scale_color_identity() +
  scale_x_continuous(breaks = 1, label = 'mm') +
  theme_bw () +
 labs(y = '', x = 'Mean eye size') +
...
grid.arrange(summer_onset_times_plot, x, ncol = 2, nrow = 1)
```
