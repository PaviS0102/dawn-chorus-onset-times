...
#interchanging GRNW (green warbler) with GRWA (greenish warbler), and changing the ebird code of Jungle Nightjar from JENI to JUNI
summer_data <- summer_data %>%
...
## Combining the data from both the seasons, and filtering species

```{r}
annotations <- bind_rows (summer_data, winter_data) #combining the data for both seasons

sp <- annotations %>%
  group_by (species) %>%
  summarize (n= n()) %>%
  filter (!n<10) %>%
  pull (species) #this gives us a list of species for which we have atleast 10 observations, i.e. 88 species

annotations <- annotations %>%
  filter (!species %in% 'UK') %>% #filtering unknown observations/species
  filter (species %in% sp) %>% #filtering the selected species
  filter (!species %in% c('SHIK', 'ISOW', 'BRBO', 'SBEO', 'CSEA')) #removing raptors, i.e. Indian Scops-Owl, Brown Boobook, Crested Serpent Eagle, Spot-bellied Eagle-Owl and Shikra from further analyses
```

## Adding scientific names and common names to the data

```{r}
species_codes <- read.csv("data/species-annotation-codes.csv") #loading the species annotation codes

species_codes <- species_codes %>%
  rename (species = species_annotation_codes) #changing the column name so that it can be appended with the annotations data
...
#This results in a dataframe with 83 species for which we have data on onset times.

write.csv (annotations, 'results/data.csv')
```
